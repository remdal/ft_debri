...
RMDLAppDelegate::~RMDLAppDelegate()
{
    std::cout << "RMDLAppDelegate destuctor" << std::endl;
    _pMtkView->release();
    _pWindow->release();
    _pDevice->release();
...
NS::Menu* RMDLAppDelegate::createMenuBar()
{
...
void RMDLAppDelegate::applicationWillFinishLaunching( NS::Notification* pNotification )
{
...
void RMDLAppDelegate::applicationDidFinishLaunching( NS::Notification* pNotification )
{
    ...
    _pMtkView->setClearColor( MTL::ClearColor::Make( 0.1, 0.1, 0.4, 1.0 ) );
    _pMtkView->setDepthStencilPixelFormat( MTL::PixelFormat::PixelFormatDepth16Unorm );
    _pMtkView->setClearDepth( 1.0f );

    _pViewDelegate = new RMDLMTKViewDelegate( _pDevice );
    _pMtkView->setDelegate( _pViewDelegate );

    _pWindow->setContentView( _pMtkView );
    _pWindow->setTitle( NS::String::string( "ft_débri.", NS::StringEncoding::UTF8StringEncoding ) );
...
bool RMDLAppDelegate::applicationShouldTerminateAfterLastWindowClosed( NS::Application* pSender )
{
    return true;
}